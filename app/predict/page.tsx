...
export default async function PredictPage() {
  ...
  const submitted = new Set(myPredictions?.map(p => p.gp_id) ?? [])

  return (
    <div>
      <h1 className="text-3xl font-bold mb-2">Previsões F1 Play</h1>
      <p className="text-gray-400 mb-8">Submete as tuas previsões antes do início de cada corrida.</p>
          ...
          return (
            <div key={gp.id} className={`card flex items-center justify-between gap-4 ${
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-400">
                    ...
                    {timeLeft && <span className="ml-2 text-f1red font-medium">⏱ {timeLeft}</span>}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 shrink-0">
                {scored && hasPred && (
                  <Link href={`/ranking/play?gp=${gp.id}`}
                    className="text-sm text-gray-400 hover:text-white underline">
                    Ver pontos
                  </Link>
                )}
                {!closed && (
                  <Link href={`/predict/${gp.id}`}
