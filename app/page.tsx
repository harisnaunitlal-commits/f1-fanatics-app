...
export default async function HomePage() {
  ...
  const { data: lastGp } = await supabase
    .from('gp_calendar')
    .select('*')
    .eq('status', 'scored')
    ...
    <div className="space-y-10">
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                ...
                <h1 className="text-4xl font-black flex items-center gap-3">
                  <span>{nextGp.emoji_bandeira}</span>
                  <span>GP {nextGp.nome}</span>
                </h1>
                <p className="text-gray-400 mt-2">
                  {new Date(nextGp.data_corrida).toLocaleDateString('pt-MZ', {
                    weekday: 'long', day: '2-digit', month: 'long', year: 'numeric',
                    hour: '2-digit', minute: '2-digit', timeZone: 'Africa/Maputo'
                  })} (Maputo)
                </p>
                {!isDeadlinePassed(nextGp.deadline_play) && (
                  <p className="text-f1red font-bold mt-1">
                    ⏱ {getTimeUntilDeadline(nextGp.deadline_play)} para o deadline
                  </p>
                )}
              </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold">
              Top 3 Global · após {lastGp.emoji_bandeira} {lastGp.nome}
            </h2>
            <Link href="/ranking" className="text-f1red text-sm hover:underline">Ver ranking completo →</Link>
          </div>
          <div className="grid grid-cols-3 gap-4">
            {top3.map((r, i) => {
              const member = r.members as { nickname: string; foto_url: string | null } | null
              return (
                <div key={r.member_email}
                  className={`card text-center ${i === 0 ? 'border-yellow-500/40 bg-yellow-900/10' : ''}`}>
                  <div className="text-3xl mb-2">
                    {i === 0 ? '🥇' : i === 1 ? '🥈' : '🥉'}
                  </div>
                  {member?.foto_url
                    ? <img src={member.foto_url} alt="" className="w-12 h-12 rounded-full object-cover mx-auto mb-2" />
          ...
          {allGps?.map(gp => (
            <div key={gp.id}
              className={`rounded-lg px-3 py-2 text-sm flex items-center gap-2 border ${
                gp.status === 'scored'   ? 'bg-green-900/20 border-green-800/30 text-gray-300' :
                gp.status === 'active'   ? 'bg-blue-900/20 border-blue-700/30 text-white' :
                gp.status === 'closed'   ? 'bg-gray-800/50 border-gray-700/30 text-gray-400' :
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="card text-center">
          ...
          <Link href="/ranking/play" className="text-f1red text-sm mt-2 block hover:underline">Ver ranking</Link>
        </div>
        <div className="card text-center">
          ...
          <Link href="/ranking/fantasy" className="text-f1red text-sm mt-2 block hover:underline">Ver ranking</Link>
        </div>
        <div className="card text-center">
          <div className="text-3xl mb-2">📊</div>
          <div className="font-bold">F1 Predict</div>
          <div className="text-xs text-gray-500 mt-1">f1predict.formula1.com · C4MIFTXAH05</div>
          <Link href="/ranking/predict" className="text-f1red text-sm mt-2 block hover:underline">Ver ranking</Link>
        </div>
      </div>
    </div>
  )
