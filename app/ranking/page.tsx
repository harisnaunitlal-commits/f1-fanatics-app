  ...
  return (
    <div>
      ...
      {scoredGpsAsc.length > 0 && (
        <GpSelector gps={scoredGpsAsc} selectedId={selectedGpId} basePath="/ranking" />
      )}

      {selectedGp && (
        <p className="text-gray-400 text-sm mb-6">
          Após R{String(selectedGp.round).padStart(2, '0')} {selectedGp.emoji_bandeira} {selectedGp.nome}
          <span className="text-gray-600"> · Score normalizado 0–100 · média das 3 ligas</span>
        </p>
      )}
                    ...
                    >
                      ...
                      <td className="py-4 px-3">
                        <div className="flex items-center gap-2.5 min-w-0">
                          {r.members?.foto_url ? (
                            <img src={r.members.foto_url} alt="" className="w-9 h-9 rounded-full object-cover shrink-0 ring-1 ring-gray-700" />
                          ) : (
                          ...
                          <div className="min-w-0">
                            <div className={`font-semibold truncate ${s.name}`}>{name}</div>
                            {r.n_ligas < 3 && (
                      ...
                      <td className="py-4 px-4 text-right">
                        <span className={`text-2xl font-bold tabular-nums ${s.score}`}>
